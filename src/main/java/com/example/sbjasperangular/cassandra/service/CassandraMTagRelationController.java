...
@RestController
@RequestMapping("rest/cassandra/m_tag_relation")
public class CassandraMTagRelationController {

	@Autowired
	CassandraMTagRelationService service;

	// curl http://localhost:8080/rest/cassandra/m_tag_relation -i -XGET
	// curl http://localhost:8080/rest/cassandra/m_tag_relation -i -XGET
	// OK
	@GetMapping
	List<CassandraMTagRelation> getCassandraMTagRelations() {
        List<CassandraMTagRelation> list = service.findAll();
        return list;
    }
//
//	// 全件抽出
